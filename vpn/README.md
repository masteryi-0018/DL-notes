...
4. DNS默认就是电脑的ip。手机要想科学上网，可以设置代理为电脑ip的10808端口，但是目前设置代理就会导致无法使用浏览器。原因是电脑不会将手机过来的流量转发到本机的特定端口，需要写一个脚本进行端口转发，之后手机也可以科学上网了。
5. 操作系统设置了代理，实际上是告诉上层，“我这有一份代理建议，你要不要用？”，上层可以选择使用这个代理（使用系统网络api），也可以选择不使用这个代理（自己设置网络）。所以浏览器可以访问YouTube，但是app不能连接，因为app可能没走代理。

## 内网穿透与反向代理

1. 家里的电脑/学校的服务器，属于内网，也就是一个局域网的设备才能访问，比如家里可以`ssh yigao@192.168.1.8`，但是外网是访问不了的。要想让外网访问到这个内网设备，就需要内网穿透工具，常见的有frp和ngrok。我们其实完全可以用ssh命令实现。
2. 内网穿透工具的原理/必要条件是：
   1. 在外网有一个服务器
   2. 这个服务器有一个公网ip
3. 内网设备和这个服务器建立一个长连接，保持通信，这样外网访问这个服务器时，服务器就可以将请求转发到内网设备上。
4. 代码：
```sh
# 内网电脑
ssh -N -f -R 6000:localhost:22 ubuntu@110.40.132.69
# 服务器不需要操作

# 服务器访问
ssh -p 6000 yigao@localhost
# 外网访问失败
ssh -p 6000 yigao@110.40.132.69
```
5. 失败的原因是服务器只监听了localhost的6000端口，而没有监听所有ip的6000端口，所以外网访问不到。解决方法是，在服务器上修改ssh配置文件`/etc/ssh/sshd_config`，将`GatewayPorts`设置为`yes`或者`clientspecified`，然后重启ssh服务，这样服务器就会监听所有ip的6000端口了。代码：
```sh
# 内网电脑（加了0.0.0.0:）
ssh -N -f -R 0.0.0.0:6000:localhost:22 ubuntu@110.40.132.69
# 服务器需要修改配置文件
...
ssh -p 6000 yigao@110.40.132.69
```
6. 附带vscode配置，`-1`是新的一次连接的，原本的是2次连接，输入密码也只需要一次就可以，原本的需要2次：
```sh
Host home-machine
    HostName localhost              # 这里填什么很关键，看下方说明
    Port 6000                       # 你之前配置的远程端口 (比如 frp 的 6000)
    User yigao                      # 你家电脑的登录用户名
    ProxyJump ubuntu@110.40.132.69  # 你的云服务器信息
    # 如果 SSH 连接不稳定，可以加下面两行保持连接
    ServerAliveInterval 60
    ServerAliveCountMax 3

Host home-machine-1
    HostName 110.40.132.69
    Port 6000
    User yigao
    # 如果 SSH 连接不稳定，可以加下面两行保持连接
    ServerAliveInterval 60
